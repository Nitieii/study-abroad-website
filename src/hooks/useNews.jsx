...
const useNews = () => {
  ...
  const { news, cultures, isLoading, totalPage } = useSelector((state) => state.news);
  ...
  const handleGetCultures = async (page, cat) => {
    dispatch(HANDLE_LOADING(true));
    try {
      const res = await axiosInstance.get(
        GET_API({ page: page, cat: cat }).getCultures
      );
      if (res.data.status === "success") {
        dispatch(GET_TOTALPAGE(res.data.totalPage))
        if (page === 1) {
          ...
          const newArray = [...cultures, ...res.data.posts ]
          dispatch(GET_NEWS(newArray))
        }
      }
      dispatch(HANDLE_LOADING(false));
